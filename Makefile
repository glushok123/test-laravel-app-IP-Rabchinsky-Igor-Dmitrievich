...
PHP_CLI_SERVICE=test-php-cli
NODE_CLI_SERVICE=test-node-cli
MARIA_DB_CONTAINER=test-mariadb
SPHINX_CONTAINER=test-sphinx
...
docker-build: \
	docker-build-test-nginx \
	docker-build-test-php-fpm \
	docker-build-test-php-cli

docker-build-test-nginx:
	@docker build -t test-nginx:${IMAGE_TAG} ./docker/${ENV}/nginx

docker-build-test-php-fpm:
	@docker build -t test-php-fpm:${IMAGE_TAG} ./docker/${ENV}/php-fpm

docker-build-test-php-cli:
	@docker build -t test-php-cli:${IMAGE_TAG} ./docker/${ENV}/php-cli
